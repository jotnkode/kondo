use std::{io::{Read, Write}, os::fd::AsRawFd, path::{Path, PathBuf}};
...
async fn open_task_editor(task: Task) -> Result<Task, Error> {
    let content = format!("[{}]\n{}", task.deadline.format("%Y-%m-%d").to_string(), task.content);
    ...
    cmd.arg(file_name.as_os_str().to_str().expect("Can't convert path to string"));
    let mut vim = cmd.spawn().expect("Can't open editor.");
    let _ = vim.wait().await;
    let updated_content = fs::read_to_string(file_name.as_os_str().to_str().unwrap()).await.expect("Couldn't read file.");
...
async fn add(date: &Option<NaiveDate>, content: &Option<String>) {
    let cfg = crate::config::Configuration::new();
    let days = Days::new(cfg.kondo.default_deadline.parse::<u64>().expect("default_deadline is non-numeric"));
    if *date == None && *content == None {
        let deadline = Utc::now().naive_utc().date().checked_add_days(days).expect("Invalid deadline date.");
        let mut task = Task::new(deadline, "");
        task = open_task_editor(task).await.expect("Can't get task from editor");
        println!("{:?}", task);
    } else {
...
#[tokio::main]
async fn main() {
    let pool = sqlx::SqlitePool::connect_lazy("sqlite:kondo.db").unwrap();
